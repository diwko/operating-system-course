...
void sig_handler_new(int sig) {
    fprintf(stderr, "PID: %d, TID: %ld - SIGNAL RECEIVED - NEW HANDLER\n", getpid(), gettid());
}

void* thread_action(void *arguments) {
    if(signal(SIGUSR1, sig_handler_new) == SIG_ERR) {
        perror("signal error");
    }
    if(signal(SIGTERM, sig_handler_new) == SIG_ERR) {
        perror("signal error");
    }
